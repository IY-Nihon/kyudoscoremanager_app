...
test('チームが3つ以上でも、区切りごとに切り替わる', () => {
  const 並び = [人('a'), 区切り('s1', 'A大'), 人('b'), 人('c'), 区切り('s2', 'B大'), 人('d')];
  const r = チームを割り当てる(並び);
...
test('射手を作り直す所が、teamName を落とさない', () => {
  ...
  const place = [];
  // 読める形に直したので true / false と 射手.isSeparator。前の !0 / !1 と e.isSeparator も受ける
  const re =
    /isSeparator:\s*(?:(?:!0|true) === [^\s.]+\.isSeparator|[^\s.]+\.isSeparator \|\| (?:!1|false))/g;
  let m;
...
test('射位を割り振る：区切りごとに大前から数え直し、計は数えない', () => {
  const { 射位を割り振る } = require('../src/teamGrouping');
  const 並び = [
    { id: 'a' },
    { id: 'b' },
    { id: 't', isTotalCalculator: true },
    { id: 's', isSeparator: true, teamName: '---' },
    { id: 'c' },
    { id: 'd' },
    { id: 'e' },
    { id: 's2', isSeparator: true },
    { id: 'f' },
  ];

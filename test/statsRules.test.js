...
// ── 一射・一手のとき（1本だけ／2本だけ引いた記録）─────────────────
// 「人たちで一本しか引いていないとき」の的中率が見たい（2026-09-14 に使う人が求めた）。
// 射数の設定は4刻みなので、行（射手）に引いた印が1つなら一射、2つなら一手と見る。
// 一手の中なら1本目が甲矢・2本目が乙矢と決まるので、どちらを抜いたかを言える。

const { 一手の型を並べる, 一手の矢の名前, 一手の呼び名 } = require('../src/statsRules');

const 行 = (marks, extra) => Object.assign({ memberId: 'm1', marks }, extra || {});

test('一射のとき：1本しか引いていない行だけを数える', () => {
  const r = 集計(
    [
      { archers: [行(['○', '', '', ''])] },
      { archers: [行(['', '×', '', ''])] },
  ...
  assert.deepStrictEqual(r.一射, { shots: 2, hits: 1 });
  assert.deepStrictEqual(r.型, { '○○○×': 1 });
});

test('一手のとき：2本だけ引いた行を数え、型は甲矢・乙矢の順', () => {
  const r = 集計(
    [
      { archers: [行(['○', '×', '', ''])] },
      { archers: [行(['×', '○', '', ''])] },
  ...
  assert.deepStrictEqual(r.一手, { shots: 6, hits: 4, 型: { '○×': 1, '×○': 1, '○○': 1 } });
  assert.deepStrictEqual(r.一射, { shots: 0, hits: 0 });
});

test('一射・一手のとき：交代した分は、その人の射だけを数え、型は2本とも同じ人のときだけ', () => {
  // 1本目は本人、2本目は交代太郎（m9）
  const 記録 = {
    archers: [行(['○', '×', '', ''], { substitutions: { 1: '交代太郎' }, substitutionIds: { 1: 'm9' } })],
  };
  assert.deepStrictEqual(集計([記録], 'm1').一手, { shots: 1, hits: 1, 型: {} });
  assert.deepStrictEqual(集計([記録], 'm9').一手, { shots: 1, hits: 0, 型: {} });
});

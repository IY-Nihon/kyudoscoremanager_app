...
const 名簿 = [
  { id: 'm1', name: '山田 太郎', isAlumni: false },
  { id: 'm2', name: '佐藤 次郎', isAlumni: false },
  { id: 'm3', name: '†鈴木 三郎†', isAlumni: false },
  { id: 'm4', name: '渡辺 四郎', isAlumni: false },
  { id: 'm5', name: '渡邉 五郎', isAlumni: false },
  { id: 'm6', name: '田中 六郎', isAlumni: false },
  { id: 'a1', name: '飯田 七郎', isAlumni: true },
];

test('roster が名簿の表記なら、その人に寄る（記号付きの表記も）', () => {
  assert.strictEqual(名簿で名寄せ('山田', '山田', 名簿).match.id, 'm1');
  assert.strictEqual(名簿で名寄せ('鈴木', '†鈴木', 名簿).match.id, 'm3');
  assert.strictEqual(名簿で名寄せ('山田', '山田(太)', 名簿).match.id, 'm1');
  assert.strictEqual(名簿で名寄せ('山田', '山田 太郎', 名簿).match.id, 'm1');
});
...
test('roster が付いていても、読めた字が名簿の姓と違うなら「もしかして」で止める', () => {
  // 「田」だけ読めて Gemini が 田中 に寄せた／「砂原」を 笹原 に寄せた、は人に決めてもらう
  const r = 名簿で名寄せ('田', '田中', 名簿);
  assert.strictEqual(r.status, 'ambiguous');
  assert.deepStrictEqual(r.options.map((o) => o.id), ['m6']);
  assert.strictEqual(名簿で名寄せ('山由', '山田', 名簿).status, 'ambiguous');
  // 姓と同じなら決まり（氏名まで読めていても）
  assert.strictEqual(名簿で名寄せ('山田太郎', '山田', 名簿).status, 'matched');
});
...
test('同じ人に2行が寄ったら、読めた文字が名簿と同じ行を残し、ほかは「もしかして」に落とす', () => {
  const rows = [
    { rawText: '山田', status: 'matched', match: 名簿[0] },
    { rawText: '佐藤', status: 'matched', match: 名簿[1] },
    // 相手校の「山本」を「山田」と読み違えた行（roster も 山田）
    { rawText: '山由', status: 'matched', match: 名簿[0] },
  ];
  const 出 = 重なりを外す(rows);
  ...
  assert.strictEqual(出[2].rawText, '山由');
});

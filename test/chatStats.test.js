...
test('名前で選ぶ：同じ名前が先、無ければ含む・含まれる。2 人以上当たれば決めない', () => {
  const 名簿 = [人('山田 太郎'), 人('山田 花子'), 人('☆鈴木 一郎☆'), 人('田')];
  assert.equal(名前で選ぶ(名簿, '鈴木').人.name, '☆鈴木 一郎☆');
  ...
  const 記録たち = [
    {
      date: 1,
      archers: [
        { memberId: 'n', name: '☆鈴木 一郎☆', marks: ['○', '×'] },
        { memberId: 't', name: '高橋', marks: ['○', '○'] },
      ],
    },
  ];
  const r = 射位ごとの成績([], 記録たち, { 名前たち: ['鈴木'] });
  assert.deepEqual(
    r.一覧.map((x) => x.名前),
    ['☆鈴木 一郎☆']
  );

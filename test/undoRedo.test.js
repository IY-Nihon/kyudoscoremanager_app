...
test('共有履歴：番号は取れて中身がまだ着いていない控えも、少し待って取り消す', async () => {
  // 積むときは番号（history_len）を先に取り、中身はそのあとで書く。2 台以上が同時に
  // 入れた直後に取り消すと、番号の先が空のことがあり、押しても何も起きなかった
  //（2026-09-26 に 3 台の e2e で 4 回に 1 回）
  const A = ライブの端末(undefined, 三人());
  A.store.getState().toggleMark('a1', 0);
  await 待つ(10);
  assert.equal(印(A.store), '○', '前提：入っている');
  // 相手の 1 手ぶん、番号だけが先に進んだ形を作る
  const 状態の道 = `live_sessions/${検査の合言葉}/${ライブ名}/state`;
  const 控えの道 = `live_history/${検査の合言葉}/${ライブ名}`;
  const 自分の手 = A.ライブ.値(`${控えの道}/0`);
  A.ライブ.置く(`${状態の道}/history_len`, 2);
  A.ライブ.置く(`${状態の道}/history_max`, 2);
  const 押した = A.store.getState().sharedUndo(-1);
  await 待つ(300);
  // 相手の中身が遅れて着く（自分の ○ を消す手と同じ形にしておく）
  A.ライブ.置く(`${控えの道}/1`, 自分の手);
  await 押した;
  assert.equal(印(A.store), '', '中身が着いたあとも取り消していない');
});

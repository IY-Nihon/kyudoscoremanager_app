...
// ──────────────────────────────────────────────────────────────
test('送り直し：送っている最中の編集にも「同期済み」が付く（既知の穴）', async () => {
  // 送信が終わったあと、そのときの手元をそのまま「同期済み」にしている。
  // 送信中に編集すると、その新しい内容まで送信済み扱いになり、
  // 送り直しの対象から外れてクラウドへ届かないままになる。
  // 記録の保存や編集では照合を入れて塞いだが、ここだけ残っている。
  // いまの実装をそのまま写した検査。直したらこれを裏返すこと。
  const { store, 雲 } = await 用意();
  store.setState({ sessions: [記録({ syncStatus: '未同期' })] });
  ...
  assert.equal(記録を見る(store).syncStatus, '同期済み', '★送っていないのに送信済み扱い');
  雲.状態.遅延 = 0;
});

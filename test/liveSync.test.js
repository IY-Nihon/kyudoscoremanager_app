...
// ──────────────────────────────────────────────────────────────
test('○×：届いた盤面を正にする。雲の行が古い内容に変わったら、手元もそれに合わせる（食い違ったまま残さない）', async () => {
  // 前は「手元の行のほうが新しければ手元を残す」決まりで、この検査もそれを確かめていた。
  // それでは雲と手元が食い違ったまま残り、ほかの端末と見え方が違ってしまう
  //（2026-09-26 に 2 台の連打で再現した不具合と同じ形）。いまは ○× は届いた盤面を正にし、
  // 古い行で相手の手を消さないことは、送る側（変わったますだけを書く）で守る（下の検査）
  const 主 = 端末(null, [射手()]);
  await 主.store.getState().startLiveSync(ライブ名);
  ...
  assert.equal(手元の射手(参.store).marks[0], '', '雲の盤面に合わせていない（食い違ったまま）');
  assert.equal(手元の射手(参.store).lastModified, 入れた日時, '日時が巻き戻った');
});

test('○×：2 台が同じ射手の別のますを同時に入れても、両方に両方の手が出る（行まるごとで選ばない）', async () => {
  // 2026-09-26 に 2 台の e2e で再現した不具合。雲には 1 ますずつ書くので両方の手が載るのに、
  // 受け取る側が射手の行まるごとを新しいほうに決めていたため、片方の端末に相手の手が出なかった
  const 主 = 端末(null, [射手()]);
  await 主.store.getState().startLiveSync(ライブ名);
  const 参 = 端末(主.ライブ, []);
  参.store.getState().joinLiveSync(ライブ名);
  await 待つ(20);
  // 届くのを遅らせて、互いの手を受け取る前に入れる
  主.ライブ.状態.遅延 = 30;
  主.store.getState().toggleMark('a1', 0);
  参.store.getState().toggleMark('a1', 2);
  await 待つ(120);
  主.ライブ.状態.遅延 = 0;
  for (const [名, 端] of [['主', 主], ['参', 参]]) {
    assert.deepStrictEqual(手元の射手(端.store).marks, ['○', '', '○', ''], `${名}に両方の手が出ていない`);
  }
  assert.deepStrictEqual(主.ライブ.値(道).marks_by_id.a1, ['○', '', '○', '']);
});

test('○×：盤面をまとめて送る操作でも変わったますだけを書く（受け取る前の相手の手を古い行で消さない）', async () => {
  const 主 = 端末(null, [射手({ marks: ['', '×', '', ''] })]);
  await 主.store.getState().startLiveSync(ライブ名);
  const 参 = 端末(主.ライブ, []);
  参.store.getState().joinLiveSync(ライブ名);
  await 待つ(20);
  // 参加者の ○ が主催者へ届く前に、主催者がその人の ○× を消す（盤面をまとめて送る操作）
  主.ライブ.状態.遅延 = 30;
  参.store.getState().toggleMark('a1', 0);
  主.store.getState().clearArcherMarks('a1');
  await 待つ(120);
  主.ライブ.状態.遅延 = 0;
  // 主催者が消したのは自分に見えていた × だけ。届いていなかった ○ は残り、2 台がそろう
  assert.deepStrictEqual(主.ライブ.値(道).marks_by_id.a1, ['○', '', '', ''], '古い行で相手の手を消した');
  for (const [名, 端] of [['主', 主], ['参', 参]]) {
    assert.deepStrictEqual(手元の射手(端.store).marks, ['○', '', '', ''], `${名}の見え方が雲と違う`);
  }
});

...
// 履歴・分析は、初めての人には空の画面しか出ない。そこだけ見本を添える
test('見本：記録が無いときだけ、履歴と分析に見本を添える', () => {
  const 見本たち = (記録数) => {
    const { 基本, 続き } = 手順を作る('group', { 部員数: 3, 記録数 });
    return [...基本, ...続き].map((h) => h.見本).filter(Boolean);
  };
...
test('案内：すでに部員や記録があるときは「まだ無い」と言わない', () => {
  ...
  assert.ok(空.includes('まだメンバーが1人も登録されていません。'), '空のときは今までどおり案内する');
  assert.ok(空.includes('まだ保存していないので、いまは空です。'));

  const 使用中 = 全文('group', { 部員数: 6, 記録数: 12 });
  assert.ok(!使用中.includes('まだメンバーが1人も登録されていません。'), 'メンバーがいるのに「1人もいない」と出た');

...
export async function 板の印を読む(元, 注文) {
  ...
  for (const { 格子: g } of 板たち) {
    ...
    出.push({
      ...
      確からしさ,
      // 確かめ用。どこに格子を立てたか
      格子: { 角度: g.角度, 列: g.列.map((c) => Math.round(c.中心)), 列の見当: g.列の見当, 並びの人数: g.並びの人数, 外した強い列: g.外した強い列, 列の点: g.列の点, 行: g.行.位置.map((v) => Math.round(v)), 行の欠け: g.行の欠け, 上の字の行: g.上の字の行, 行ごとの列数: g.行ごとの列数, 印の幅: g.印の幅 },
    });
  }
  return 出;
}

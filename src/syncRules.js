 ...
 *
 * 端末の控えに入りきらず古い記録を控えから外したとき（src/localTrim.js）も全件にする。
 * 外した記録は雲では変わっていないので差分に出てこず、差分で起動すると履歴から消えて見える。
 *
 * @param {{団体: string, 境目: object|null, 全部そろえた時刻: number, 記録の数: number, 部員の数: number, 記録を外した?: boolean, 今?: number}} 様子
 * @returns {'全部'|'差分'}
 */
function 起動のしかた(様子) {
  const { 団体, 境目, 全部そろえた時刻, 記録の数, 部員の数, 記録を外した } = 様子 || {};
  ...
  if (記録を外した) return '全部'; // 控えから外した古い記録は、全件でしか戻らない
  return '差分';
}

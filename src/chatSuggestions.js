...
function 質問例(材料) {
  ...
  // 聞くだけでなく、頼める操作もある（部員の追加は承認のカードが出る。画面も開ける）。
  // 出さないと「質問に答えるだけ」と思われて使われない
  足す('操作', '新しい部員を追加して（名前・学年・性別）');
  足す('操作', '分析の画面を開いて');
  足す('使い方', 'ライブ記録の使い方を教えて');
  足す('使い方', '写真から記録を読み取るには？');
  足す('使い方', '記録表を横向きにするには？');

  // 全部出すと画面が埋まり、挨拶も見えなくなる。分類ごとに1件ずつ拾って
  // 幅を持たせ、6件までにする（分類が5つになったので、成績の2件目が残るぶんを足した）。
  // 続きは打ちながらの候補で出る
  if (素.件数 === Infinity) return 例; // 打ちかけの候補は全部から探す
  const 上限 = Number.isFinite(素.件数) && 素.件数 > 0 ? 素.件数 : 6;
  const 出た = [];
  const 使った = new Set();
  // まず分類ごとに1件（並びの順＝大事な順）
  for (const x of 例) {

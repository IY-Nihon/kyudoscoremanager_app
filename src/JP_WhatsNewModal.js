...
// 「今後表示しない」を押した人に二度と出ない（検査で見張っている）
const 最後に配信した版 = '2026-08-28-01';
const NOTICE_VERSION = '2026-08-28-02';
...
const NOTICE_ITEMS = [
  {
    date: '2026/08/28',
    版: '2026-08-28-02',
    title: 'AIアシスタントも、集計に含めない記録を数えなくなりました',
    points: [
      '順位は「集計に含めない」にした記録を外していましたが、AIアシスタントに人を指定して聞いたときと、射位ごとの的中率では外していませんでした。',
      '同じ話の中で、同じ人の射数と的中率が食い違うことがありました。どちらも外すようにしました。',
    ],
  },
  {
    date: '2026/08/28',
    版: '2026-08-28-02',
    title: '比較の組み合わせを、名前を付けて残せます',
    points: [
      '個人の詳細で「一年生の4人」のような組み合わせを毎回選び直すのが手間でした。',
      '相手を選んだあとに名前を付けて保存すると、次からは一度押すだけで呼び出せます。団体ごとに12個まで残せます。',
      '残すのは部員だけで、名簿から抜けた人は呼び出したときに外し、その旨をお知らせします。',
      'この控えはお使いの端末にだけ残ります。ほかの人や別の端末には出ません。',
    ],
  },
  {
    date: '2026/08/28',
    版: '2026-08-28-02',
    title: 'お問い合わせが、メールアドレス無しでも送れます',
    points: [
      'メールアドレスの入力を必須にしていたため、返事が要らない報告まで送りにくくなっていました。',
      '空のままでも送れるようにしました。書いていただいた場合だけ、こちらから返事をお送りします。',
    ],
  },
  {
    date: '2026/08/28',
    版: '2026-08-28-02',
    title: '不具合が起きたとき、こちらに届くようにしました',
    points: [
      '「同期エラーを通知」の切り替えをなくし、同期に失敗したときはいつもお知らせするようにしました。切っていることを忘れたまま、何日も同期できていない状態になり得たためです。',
      'あわせて、不具合が起きたときは内容と直前の操作（画面の移動、保存、同期など）がこちらに自動で届くようにしました。お知らせをいただかなくても、原因を追えるようにするためです。',
      '電波が届かないときは端末に控えておき、つながったときに送ります。',
      '送るのは不具合の内容と操作の名前だけです。部員の氏名や的中の記録は含めていません。くわしくはプライバシーポリシー第4条をご覧ください。',
    ],
  },
  {
    date: '2026/08/28',

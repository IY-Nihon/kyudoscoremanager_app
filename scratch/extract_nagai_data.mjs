...
async function main() {
  try {
    ...
    for (const groupId of groupIds) {
      ...
      mSnap.forEach(d => {
        const data = d.data();
        if (data.name === "部員A" || data.name?.includes("5番")) {
          nagaiId = d.id;
        }
      });
      ...
      sSnap.forEach(d => {
        ...
        if (session.archers && Array.isArray(session.archers)) {
          session.archers.forEach((archer, archerIdx) => {
            ...
            // この行で部員Aが関与しているかチェック
            // （元の射手が部員A、または交代相手に部員Aがいる）
            let hasNagai = (archer.memberId === nagaiId || archer.name === "部員A" || archer.name?.trim().replace(/\s/g, "") === "部員A");
            if (!hasNagai) {
              // 交代メンバーに5番がいるか確認
              for (const idx of subIndices) {
                const subName = substitutions[idx] || '';
                const subId = substitutionIds[idx];
                if (subId === nagaiId || subName === "部員A" || subName.trim().replace(/\s/g, "") === "部員A") {
                  hasNagai = true;
                  break;
                }
              }
            ...
            // 1射ごとに、その射を引いたのが5番かどうかを判定してカウント
            ...
            marks.forEach((mark, shotIdx) => {
              ...
              const isNagai = (nagaiId && currentId === nagaiId) || (currentName === "部員A" || currentName.trim().replace(/\s/g, "") === "部員A");

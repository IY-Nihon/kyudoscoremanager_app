...
const { 届いた射手に合わせる, normalizeArrowLocations } = createRequire(import.meta.url)('../src/syncRules.js');
...
try {
  ...
  check(
    ...
    'このとき受信側は手元の矢所を残す（届いた射手に合わせる）'
  );

  // ── 5. 端末を2台に見立てて、本物の判断を実データに当てる ────────
  //
  // アプリの受信側と同じ順序で処理する。
  //   サーバーの値 → 受信の整形（w 相当）→ 届いた射手に合わせる → ○× を届いた盤面に → 手元の一覧
  // 整形と ○× の糊だけこの場で書き、判断は src/syncRules.js のものをそのまま使う。
  // （2026-09-26 から ○× 以外も届いた盤面を正にする。前は lastModified の新しいほう）
  const 受信の整形 = (状態) => {
  ...
  // アプリの 印を盤面に合わせる と同じく、○× は届いた盤面のものにする
  const 合わせる = (手元, 受信) => {
    const r = 届いた射手に合わせる(手元, 受信.archers, 4, 受信.本数);
    const 印 = new Map(受信.archers.map((a) => [a.id, a.marks]));
    return { archers: r.archers.map((a) => (印.has(a.id) ? { ...a, marks: 印.get(a.id) } : a)) };
  };
  ...
  // B が自分の矢所を置き（射手の一覧ごと送る）、○を1本入れて軽量送信する。
  // どちらも「手元に反映してから送る」ので、こちらも同じ順序にする
  ...
  await rt('PATCH', 道, {
    'archers/0/arrowLocations': [{ x: 1, y: 2 }, { x: 12, y: 34 }, '', ''],
    'marks_by_id/a1/3': '○',
    'archer_timestamps/a1': B時刻,
    timestamp: B時刻,
  });

  // B が自分の送信の返りを受け取っても、自分の矢所を失わない
  const B返り = 受信の整形((await rt('GET', 道)).json);
  const B2 = 合わせる(B手元, B返り);
  ...
  const A2 = 合わせる([{ ...A射手, arrowLocations: [null, { x: 12, y: 34 }, null, null] }], A受信);
  check('2台', 'A は B の○を受け取る', '○', A2.archers[0].marks[3]);
  check('2台', 'A は B の矢所も受け取る（雲と同じになる）', [{ x: 1, y: 2 }, { x: 12, y: 34 }, null, null], A2.archers[0].arrowLocations);
  ...
  const A3 = 合わせる(A2.archers, 古い受信);
  check('2台', '古い版の上書きでも矢所が残る', [{ x: 1, y: 2 }, { x: 12, y: 34 }, null, null], A3.archers[0].arrowLocations);

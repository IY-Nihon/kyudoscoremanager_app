...
for (let i = 0; i < 30; i++) {
  ...
  if (g.列の見当 === 人数 && g.行の欠け === 0) 見当++;
  else console.log(`  板${i}: 列の見当 ${g.列の見当} ≠ 人数 ${人数}  ` + g.列の点.map((x) => `${x.中心}:埋${x.埋}丸${x.丸}`).join(' '));
}
console.log(`描いた板 ${全}枚: 格子が立った ${立}枚、列と行が答えと合った ${合}枚、列の見当が人数と合い行の欠けが無い ${見当}枚`);

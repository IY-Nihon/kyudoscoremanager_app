...
export async function 形にする(明るさ, 幅, 高, 選び方, 注文) {
  ...
  for (let y0 = 0; y0 < 高; y0++) {
    for (let x0 = 0; x0 < 幅; x0++) {
      ...
      if (選び方 !== 'ぎっしり' && w2 > 幅 * 0.88 && h2 > 高 * 0.88 && !輪らしいか(積, 始, 尻, 幅, 一つ)) continue;
      かたまり.push(一つ);
    }
  }
  ...
  for (let 増えた = true; 増えた; ) {
    ...
    for (const k of かたまり) {
      ...
      const 縁 = !印が箱いっぱい && (k.左 <= 0 || k.上 <= 0 || k.右 >= 幅 - 1 || k.下 >= 高 - 1);
  ...
  if (注文 && 注文.印の幅 > 0 && 右 - 左 + 1 < 注文.印の幅 * 0.6 && 下 - 上 + 1 < 注文.印の幅 * 0.6) {
    const 輪 = 薄い輪を探す(明るさ, 幅, 高, 暗い端 + (最大 - 暗い端) * 0.8, (左 + 右) / 2, (上 + 下) / 2);
 ...
 */
function 薄い輪を探す(明るさ, 幅, 高, 境, px, py) {
  const 見た = new Uint8Array(幅 * 高);
  const 積 = new Int32Array(幅 * 高);
  let 尻 = 0;
  const 選んだ = []; // [始, 終) の並び

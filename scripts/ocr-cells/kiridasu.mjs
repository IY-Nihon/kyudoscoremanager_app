...
function 罫線で格子(生, 注文, 角度) {
  ...
  const 針を拾う = (並び, 本数, 長さの向こう, 見当の指定) => {
    ...
    let 最良 = null;
    for (let 間隔 = 見当 * 0.75; 間隔 <= 見当 * 1.25; 間隔 += 0.25) {
    ...
    const 位置 = [];
    for (let k = 0; k <= 本数; k++) 位置.push(最良.起点 + 最良.間隔 * k);
    return { 位置, 間隔: 最良.間隔, 始め: 最良.起点 };
  };
  // 見当は画の長さ÷本数（画に写っているのが印の部分だけ、という前提）。
  // 候補の端から端で取ると、印の縁の針に引かれて狂う（実測で 51 が 37 になった）。
  // マスは正方形とは限らない（実測の板で 47×55）ので、列の間隔を行に流用しない
  const 列 = 針を拾う(横, 注文.人数, 生.高);
  const 行 = 針を拾う(縦, 注文.行数, 生.幅);
  if (!列 || !行) return null;
  const 列の中心 = [];

...
const 種を振る = () => Math.floor(乱れ() * 2147483647);
...
async function 見本をあつめる(枚数) {
  ...
  for (let i = 0; i < 枚数; i++) {
    ...
    // マスごとに画像を開き直さないのは速さのため（板120枚で数十分かかっていた）
    const 板の画 = g.生.画素;
    const 板の形 = { width: g.生.幅, height: g.生.高 };
    ...
    for (let c = 0; c < g.列.length; c++) {
      for (let r = 0; r < g.行.位置.length; r++) {
        ...
        for (const [dx, dy] of ずらし) {
          ...
          let 切 = 切り取る(板の画, 板の形.width, 板の形.height, 左, 上, 半幅 * 2, 半高 * 2);
          // マスごとに、写真の崩れも混ぜる。斜めに撮った写真は格子が起こすが、
          // 起こしきれない数度の傾きと回したときのぼけが残る。暗さ・薄さ・
          // ざらつきは、そのまま網に届く。崩した本物で測ると、傾き6〜10度で
          // 96%、ざらつきで 95.6% に落ちていた（崩しなしなら100%）
          // 崩しは既定では入れない。入れて測ったら、素の当たりは 155/160 で変わらず、
          // 綺麗な写真の辻褄合わせが 320 → 316 に落ちた（確からしさが鈍って、
          // 合わせるときに正しいマスまで動かした）。強さや割合はまだ詰めていない
          if (process.env.OCR_KUZUSHI && 乱れ() < 0.6) {
            const 崩し = 乱れ() < 0.6
              ? 崩し方({ 幅: 切.幅, 高: 切.高, 回す: (乱れ() - 0.5) * 7, 台形: 乱れ() * 0.12, 種: 種を振る() })
              : null;
            const 味 = ゆがませる(

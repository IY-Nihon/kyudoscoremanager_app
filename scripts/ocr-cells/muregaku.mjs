    ...
    for (let c = 0; c < g.列.length; c++) {
      for (let r = 0; r < g.行.位置.length; r++) {
        ...
        for (const [dx, dy] of ずらし) {
          ...
          // 写真の崩れ（ざらつき・ぼけ・低解像度・暗さ）をマスに混ぜる手は、
          // 板では既定で切ってある。2度試して2度とも綺麗な写真が落ちた
          //（回転込みで 320 → 316、画質だけでも 320 → 318、4人立ちも1列落ちた）。
          // 確からしさが鈍り、辻褄合わせが正しいマスまで動かす。OCR_KUZUSHI=1 で入る
          if (process.env.OCR_KUZUSHI === '1' && 乱れ() < 0.5) {
            const 味 = ゆがませる(
              { 画: 切.画, 幅: 切.幅, 高: 切.高, 面: 1 },
              {
                ...
                締まり: 0.5 + 乱れ() * 0.6,
                ざらつき: 乱れ() < 0.6 ? 乱れ() * 45 : 0,
                ぼかし: 乱れ() < 0.5 ? 乱れ() * 切.幅 * 0.06 : 0,
                縮小: 乱れ() < 0.4 ? 0.33 + 乱れ() * 0.6 : 1,
                種: 種を振る(),
              }
            );
            切 = { 画: 味.画, 幅: 味.幅, 高: 味.高 };
